...
export const exchangeGratisReport = new ValidatedMethod({
    ...
    run(params) {
        if (!this.isSimulation) {
            ...
            if (params.date) {
                ...
                selector.exchangeGratisDate = {$gte: fromDate, $lte: toDate};
            }
            ...
            if (params.filter && params.filter != '') {
                ...
                data.displayFields = [{field: '_id'}, {field: 'exchangeGratisDate'}, {field: 'vendorName'},{field: 'vendorTelephone'}, {field: 'status'}, {field: 'sumRemainQty'}, {field: 'total'}];
            }
            ...
            let total = ExchangeGratis.aggregate(
                [
                    {
                        $match: selector
                    },
            ...
            if (exchangeGratis.length > 0) {
                let sortData = _.sortBy(exchangeGratis[0].data, '_id');
                exchangeGratis[0].data = sortData;
                data.content = exchangeGratis;
